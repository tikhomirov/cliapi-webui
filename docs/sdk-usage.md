...
- For available endpoints, see `internal/api/server.go` (route registration) and `internal/api/handlers/management/` (handlers). Your embedded server exposes them under `/v0/management` on the configured port.
