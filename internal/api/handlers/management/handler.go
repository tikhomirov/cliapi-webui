...
func NewHandler(cfg *config.Config, configFilePath string, manager *coreauth.Manager) *Handler {
	...
	noAuth := false
	if v, _ := os.LookupEnv("NO_MANAGEMENT_AUTH"); v != "" {
		noAuth = strings.TrimSpace(strings.ToLower(v)) == "true" || strings.TrimSpace(v) == "1"
	}
...
// When NO_MANAGEMENT_AUTH is set in the environment, auth is skipped entirely.
