...
// PutConfig accepts a full config as JSON, merges it over the current config
// (preserving fields that have json:"-" tags), validates and persists as YAML.
func (h *Handler) PutConfig(c *gin.Context) {
	...
	// Clone current config so fields with json:"-" are preserved
	var cfg config.Config
	if h.cfg != nil {
		yamlBytes, _ := yaml.Marshal(h.cfg)
	...
	// Validate by writing to temp file and reloading
	...
	if _, errWrite := tmpFile.Write(yamlBytes); errWrite != nil {
		_ = tmpFile.Close()
	...
	h.mu.Lock()
	defer h.mu.Unlock()
	if WriteConfig(h.configFilePath, yamlBytes) != nil {
