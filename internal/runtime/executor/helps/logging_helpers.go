...
// LogUpstream403 logs a warning when an upstream provider returns HTTP 403 Forbidden.
// This helps surface authentication/key issues that would otherwise only appear at Debug level.
func LogUpstream403(ctx context.Context, provider string, statusCode int, body []byte) {
	if statusCode != http.StatusForbidden {
		return
	}
	entry := LogWithRequestID(ctx).WithFields(log.Fields{
		"provider": provider,
	})
	summary := SummarizeErrorBody("", body)
	if summary != "" {
		entry.Warnf("upstream returned HTTP 403 Forbidden for provider %s: %s", provider, summary)
	} else {
		entry.Warnf("upstream returned HTTP 403 Forbidden for provider %s (API key may be invalid or revoked)", provider)
	}
}
