...
func (s *Service) ensureExecutorsForAuthWithMode(a *coreauth.Auth, forceReplace bool) {
	...
	case "opencode", "opencode-zen":
		s.coreManager.RegisterExecutor(executor.NewOpenCodeExecutor(strings.ToLower(strings.TrimSpace(a.Provider)), s.cfg))
	case "opencode-go":
		s.coreManager.RegisterExecutor(executor.NewOpenCodeExecutor(strings.ToLower(strings.TrimSpace(a.Provider)), s.cfg))
	default:
			...
			for i := range s.cfg.OpenAICompatibility {
				...
				if strings.EqualFold(compat.Name, compatName) {
					...
					for j := range compat.Models {
						m := compat.Models[j]
						// Use alias as primary model ID, fallback to name if alias is empty.
						// Register under BOTH alias and name so that users can test
						// with either identifier and the registry lookup succeeds.
						aliasID := m.Alias
						if aliasID == "" {
							aliasID = m.Name
						}
						...
						// Primary registration under alias
						...
						// Secondary registration under original name (if different from alias)
						if m.Name != "" && m.Name != aliasID {
							ms = append(ms, &ModelInfo{
								ID:          m.Name,
								Object:      "model",
								Created:     time.Now().Unix(),
